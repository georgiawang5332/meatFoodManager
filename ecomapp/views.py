...
class ProductDetailView(EcomMixin, TemplateView):
  template_name = "ecomapp/productdetail.html"
...
class AddToCartView(EcomMixin, TemplateView):
  ...
  def get_context_data(self, **kwargs):
    context = super().get_context_data(**kwargs)
    # get product id from requested url# 從請求的 url 中獲取產品 ID
    product_id = self.kwargs['pro_id']
    # get product# 獲取產品
    product_obj = Product.objects.get(id=product_id)

    # check if cart exists# 檢查購物車是否存在
    # cart.cartproduct_set.all().delete()#返回屬於特定公司的User對象的 QuerySet 。::https://tekshinobi.com/_set-meaning-in-django-many-to-many-relationship/  2.https://docs.djangoproject.com/en/3.2/ref/models/relations/
    ...
    if cart_id:
      ...
      # item already exists in cart# 商品已存在於購物車中
      if this_product_in_cart.exists():
        cartproduct = this_product_in_cart.last()
        cartproduct.quantity += 1
        cartproduct.subtotal += product_obj.selling_price  # Product
        cartproduct.save()
        cart_obj.total += product_obj.selling_price  # Cart, Product
        cart_obj.save()  # Cart
      # new item is added in cart# 新商品加入購物車
      else:
      ...
      cart_obj = Cart.objects.create(total=0)  # Cart
      self.request.session['cart_id'] = cart_obj.id
      cartproduct = CartProduct.objects.create(
        cart=cart_obj, product=product_obj, rate=product_obj.selling_price, quantity=1,
        subtotal=product_obj.selling_price)
      cart_obj.total += product_obj.selling_price  # Cart
      cart_obj.save()  # Cart
...
class ManageCartView(EcomMixin, View):
  def get(self, request, *args, **kwargs):
    ...
    cart_obj = cp_obj.cart  # Cart

    if action == "inc":
      cp_obj.quantity += 1
      cp_obj.subtotal += cp_obj.rate
      cp_obj.save()
      cart_obj.total += cp_obj.rate  # Cart
      cart_obj.save()  # Cart
    elif action == "dcr":
      cp_obj.quantity -= 1
      cp_obj.subtotal -= cp_obj.rate
...
class KhaltiRequestView(View):
  def get(self, request, *args, **kwargs):
    ...
    return render(request, "ecomapp/khaltirequest.html", context)
...
class EsewaRequestView(View):
  def get(self, request, *args, **kwargs):
    ...
    return render(request, "ecomapp/esewarequest.html", context)
...
class CustomerLoginView(FormView):
  ...
  # form_valid 方法是一種post方法，在createview formview和updateview中可用
  def form_valid(self, form):
...
class ContactView(EcomMixin, TemplateView):
  template_name = "ecomapp/contactus.html"
...
class AdminLoginView(FormView):
  template_name = "ecomapp/adminpages/adminlogin.html"
  form_class = CustomerLoginForm
...
class AdminHomeView(AdminRequiredMixin, TemplateView):
  template_name = "ecomapp/adminpages/adminhome.html"
...
class AdminOrderDetailView(AdminRequiredMixin, DetailView):
  template_name = "ecomapp/adminpages/adminorderdetail.html"
  model = Order
  context_object_name = "ord_obj"
...
class AdminOrderListView(AdminRequiredMixin, ListView):
  template_name = "ecomapp/adminpages/adminorderlist.html"
...
class AdminProductListView(AdminRequiredMixin, ListView):
  template_name = "ecomapp/adminpages/adminproductlist.html"
  queryset = Product.objects.all().order_by("-id")
  context_object_name = "allproducts"


class AdminProductCreateView(AdminRequiredMixin, CreateView):
  template_name = "ecomapp/adminpages/adminproductcreate.html"
